...
def main():
    ...
    if "--help" in sys.argv or "-h" in sys.argv:
        parser = argparse.ArgumentParser(
            description="Launches Rhino and fixes the given mesh file(s) according to the options below. All options are optional, the command works as long as the current working directory only contains mesh files"
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--output",
            type=str,
            help="Output folder, defaults to current folder if not given. Needs to be a folder directory, not a file. File(s) will always be saved as the extension of the input file",
        )
        parser.add_argument(
            "--preprocessing",
            help="Type of preprocessing, either 'shrinkwrap', 'fixholes', or 'fixshell'. Defaults to 'shrinkwrap' (without apostrophes)",
        )
        parser.add_argument(
            "--smoothing",
            type=float,
        ...
        parser.add_argument(
            "--edgelength", type=float, help="Sets the target edge length of the shrinkwrap function. Defaults to 1 mm. "
        )
        parser.add_argument(
            "--keepopen",
            action="store_true",
    ...
    if "Rhino.exe" not in sys.executable:
        ...
        parser.add_argument(
            ...
            type=str,
            default=os.getcwd(),
            help="Path to the input file or folder, defaults to current folder if not given",
        )
        parser.add_argument(
            "--output",
            type=str,
            default=os.getcwd(),
            help="Output folder, defaults to current folder if not given. Needs to be a folder directory, not a file",
        )
        parser.add_argument(
            "--preprocessing",
            default="shrinkwrap",
        ...
        parser.add_argument(
            ...
            type=float,
            default=1,
            help="Sets the target edge length of the shrinkwrap function. Defaults to 1 mm. ",
        )
        parser.add_argument(
            "--keepopen",
            action="store_true",
    ...
    if len(inputPath.split(".")) == 2:
        ...
        preprocessedMesh = tools.PreProcessing(
            orgMesh,
            resultsPath,
            filename,
            saveOutput=True,
