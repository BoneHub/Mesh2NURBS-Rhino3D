...
def stl2cad(mesh, targetEdgeLength=2, adaptiveSize=100, deleteInputs=True, subd=True, type="1"):
    ...
    while retry:
        ...
        rs.Command(
            f"_-NoEcho _-QuadRemesh TargetEdgeLength={targetEdgeLength} AdaptiveSize={adaptiveSize} Enter"
        )  # quad remeshing
        quad = rs.FirstObject()
...
def meshDistanceCalculator(mesh1, mesh2):
    ...
    for i in range(0, len(faces1)):
        ...
        centers1.append(
            [
                (current_face[0, 0] + current_face[1, 0] + current_face[2, 0]) / 3,
                (current_face[0, 1] + current_face[1, 1] + current_face[2, 1]) / 3,
                (current_face[0, 2] + current_face[1, 2] + current_face[2, 2]) / 3,
            ]
        )
    ...
    for i in range(0, len(faces2)):
        ...
        centers2.append(
            [
                (current_face[0, 0] + current_face[1, 0] + current_face[2, 0] + current_face[3, 0]) / 4,
                (current_face[0, 1] + current_face[1, 1] + current_face[2, 1] + current_face[3, 1]) / 4,
                (current_face[0, 2] + current_face[1, 2] + current_face[2, 2] + current_face[3, 2]) / 4,
            ]
        )
    ...
    for i in range(0, len(centers2)):
        ...
        if intersect > 0 and intersect < cutoffValue:
            ...
            dist.append(
                (
                    (centers2[i][0] - hit_point.X) ** 2
                    + (centers2[i][1] - hit_point.Y) ** 2
                    + (centers2[i][2] - hit_point.Z) ** 2
                )
                ** 0.5
            )
...
def fullPipeline(
    ...
    if preProcess:
        # pre-process
        shrinkWrappedMesh = PreProcessing(
            mesh=originalMesh,
...
def heatmap(mesh1, mesh2, inputPath, outputPath, plyPath, edgeConversion=2, quads=True, filename=None):
    ...
    max_error_marker = Sphere(pos=max_dist_point, r=SHRINK_POINT_SIZE * MAX_ERROR_MARKER_SIZE_FACTOR, c="red")
...
def mesh_to_subd(mesh_id):
    ...
    rs.Command(
        "_-ToSubD _UseMesh=ControlPoints _MeshCreases=No _MeshCorners=No _UseSurface=Location _SurfaceCorners=No _DeleteInput=No _Enter"
    )
    time.sleep(0.5)
